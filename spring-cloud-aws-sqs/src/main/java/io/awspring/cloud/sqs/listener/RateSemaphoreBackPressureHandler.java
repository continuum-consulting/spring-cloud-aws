 ...
 * @since 3.0
 */
public class RateSemaphoreBackPressureHandler implements BatchAwareBackPressureHandler, IdentifiableContainerComponent {

	private static final Logger logger = LoggerFactory.getLogger(RateSemaphoreBackPressureHandler.class);
	...
	private RateSemaphoreBackPressureHandler(Builder builder) {
		...
		this.semaphore = new TimedSemaphore(10, TimeUnit.SECONDS, totalPermits);
		...
		logger.error("SemaphoreBackPressureHandler created with configuration {} and {} total permits",
	...
	private int tryAcquirePartial() throws InterruptedException {
		int availablePermits = this.semaphore.getAvailablePermits();
	...
	private int requestInLowThroughputMode() throws InterruptedException {
		...
		logger.debug("Trying to acquire full permits for {}. Permits left: {}", this.id,
				this.semaphore.getAvailablePermits());
	...
	private boolean tryAcquire(int amount, CurrentThroughputMode currentThroughputModeNow) throws InterruptedException {
		...
		boolean hasAcquired = true;
		for (int i = 0; i < amount && hasAcquired; i++) {
			hasAcquired = this.semaphore.tryAcquire();
		}
		if (hasAcquired) {
			logger.trace("{} permits acquired for {} in TM {}. Permits left: {}", amount, this.id,
					currentThroughputModeNow, this.semaphore.getAvailablePermits());
		}
		else {
	...
	public void releaseBatch() {
		System.out.println("releaseBatch");
	}
	...
	public boolean drain(Duration timeout) {
		logger.debug("Waiting for up to {} seconds for approx. {} permits to be released for {}", timeout.getSeconds(),
				this.totalPermits - this.semaphore.getAvailablePermits(), this.id);
